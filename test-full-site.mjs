...
async function test() {
  ...
  for (const p of pages) {
    console.log(`\n=== 测试: ${p.name} (${p.url}) ===`);
    try {
      const response = await page.goto(BASE_URL + p.url, { waitUntil: 'networkidle', timeout: 15000 });
  ...
  console.log('\n=== 测试: 组合详情页 ===');
  try {
    await page.goto(BASE_URL + '/portfolios', { waitUntil: 'networkidle' });
    // 找到第一个组合链接
  ...
  try {
    ...
    if (firstPositionLink) {
      ...
      await page.goto(BASE_URL + '/positions/cmn89n0n1000kyxwfkxfrc8j4', { waitUntil: 'networkidle' });
      const title = await page.title();
      console.log(`  详情页标题: ${title}`);
    }
  } catch (err) {
  ...
  console.log('\n=== 测试: 论题详情页 ===');
  try {
    await page.goto(BASE_URL + '/theses', { waitUntil: 'networkidle' });
