...
**结果摘要**:
- 蔚来(NIO): bearish影响, impactScore=7, 健康度 65 (-17), 假设 weakened
- 触发因素: 月交付量环比下降23%，核心假设面临挑战
- 全量未读 3条：1 urgent(NIO) + 1 watch(00883) + 1 info(00700)
- `level=important,urgent` 联合查询返回 0（历史已知 API 逗号解析问题）

**服务器**: 62.234.79.188:4000

---

## 2026-04-15 12:07 执行结果
...
**结果摘要**:
- 全量未读 3条：info(腾讯00700) + watch(中海油00883) + urgent(蔚来NIO)
- 蔚来 urgent alert `cmna1yllj` 已分析 10+ 次，POST /analyze 返回"Alert不存在"（可能已软删除）
- `level=important,urgent` 联合查询仍返回 0（历史已知 API 问题）
- 无新的 high/medium 级别 alert 需处理

**服务器**: 62.234.79.188:4000

---

## 2026-04-13 23:56 执行结果
...
**结果摘要**:
- 蔚来(NIO): bearish, impactScore=7, 健康度 65 (-15), 假设 weakened
- 触发因素: 月交付量环比↓23%，核心业务增长预期受影响
- followUp: 下月交付量数据及营收/利润报告
- 全量未读：1 urgent(NIO) + 0 important
- 注意: 本地 localhost:4000 未运行，使用远程 62.234.79.188:4000

**服务器**: 62.234.79.188:4000

---

## 2026-04-13 10:35 执行结果
...
**结果摘要**:
- 蔚来(NIO): neutral影响, impactScore=5, 健康度 80 (±0), 假设 intact
- 触发因素: 月交付量环比-23%，建议密切关注
- 全量未读 3条：1 urgent(NIO) + 1 watch(00883) + 1 info(00700)
- `level=important,urgent` 联合查询返回 0（历史已知 API 逗号解析问题）
- **差异**: 新鲜分析结果为 neutral/intact（healthScore 80），与历史 bearish 版本（65）不同

**服务器**: localhost:4000

---

## 2026-04-13 07:07 执行结果

**执行状态**: ✅ 成功（无新增）

**分析数量**: 1条 (蔚来 urgent) — 与历史一致，跳过实际重复分析

**结果摘要**:
- 蔚来(NIO): bearish影响, impactScore=7, 健康度 65 (-15), 假设 weakened
- 全量未读 3条：1 urgent(蔚来) + 1 watch(00883) + 1 info(00700)
- `level=important,urgent` 联合查询返回 0（历史已知 API 逗号解析问题）
...
**结果摘要**:
- 蔚来(NIO): bearish影响, impactScore=7, 健康度基准65 (-17), 假设 weakened
- 触发因素: 月交付量环比下降23%
- 全量未读 3条：1 urgent(蔚来) + 1 watch(00883) + 1 info(00700)
- `level=important,urgent` 联合查询返回 0（历史已知 API 逗号解析问题）
...
## 2026-04-12 00:10 执行结果

**执行状态**: ⚠️ 系统资源不足

**分析数量**: 0条

**问题**: `zsh: fork failed: resource temporarily unavailable` — 系统无法 fork 新进程，curl 命令无法执行

**分析**: 当前时间 00:10，距上次执行（00:05 记忆系统优化）仅5分钟，agent 可能存在资源泄漏或服务器内存不足。建议检查 PM2 进程和 agent 进程数量。

**服务器**: localhost:4000

---

## 2026-04-11 22:07 执行结果
...
- 蔚来(NIO): bearish影响, impactScore=7, 健康度 65 (-15), 假设 weakened
- 触发因素: 月交付量环比下降23%
- `level=important,urgent` 联合查询返回 0（历史已知 API 参数解析问题），urgent alert 通过全量未读查询筛出
- 3条未读: 1 urgent(NIO) + 1 watch(00883) + 1 info(00700)

**服务器**: 62.234.79.188:4000

---

## 2026-04-11 18:04 执行结果
...
## 2026-04-11 16:02 执行结果

**执行状态**: ✅ 成功（无新增alert）

**分析数量**: 0条（无新增）

**说明**: 101条未读 alerts 均为历史积压（2026-04-08 Yahoo RSS风暴），与 10:44/15:43 执行完全一致，连续 3 天 0 新增，无需重复分析。
